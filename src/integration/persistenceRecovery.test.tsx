...
describe('persistence recovery integration', () => {
  ...
  it('conserves sequential create, edit, and delete mutations in one validated envelope', async () => {
    ...
    const created = await incomes.create({ saleDate: '2026-08-10' as never, amountMinor: 25000 as never, note: 'Created' })
    const updated = await incomes.update(created.id, { saleDate: '2026-08-10' as never, amountMinor: 30000 as never, note: 'Edited' })
    ...
    expect((await incomes.findAll()).map((income) => income.saleDate)).toEqual(['2026-08-09', '2026-08-08'])
