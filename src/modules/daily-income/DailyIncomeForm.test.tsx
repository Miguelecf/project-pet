...
describe('DailyIncomeForm', () => {
  ...
  it('creates a valid positive integer income with optional trimmed note and current currency disclosure', async () => {
    ...
    await screen.findByText('Moneda registrada: USD')
  ...
  it('rejects zero, future, and duplicate dates before persistence', async () => {
    ...
    await screen.findByText('Moneda registrada: USD')
    fireEvent.change(screen.getByLabelText('Fecha de venta'), { target: { value: '2026-08-10' } })
    fireEvent.change(screen.getByLabelText('Monto (unidades mínimas)'), { target: { value: '0' } })
    fireEvent.click(screen.getByRole('button', { name: 'Guardar' }))
  ...
  it('edits without changing the historical currency snapshot', async () => {
    ...
    await screen.findByText('Moneda registrada: USD')
    fireEvent.change(screen.getByLabelText('Monto (unidades mínimas)'), { target: { value: '700' } })
    fireEvent.click(screen.getByRole('button', { name: 'Guardar' }))
