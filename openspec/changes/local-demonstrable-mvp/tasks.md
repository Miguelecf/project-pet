...
M0.3a, M0.3b, M1.1, M1.2, M2.1, and M2.2 are complete; M2.3 is next.
...
- [ ] 3.4.1 RED: Write `src/modules/invoices/usePayments.test.ts` — register payment calls repository, void calls repository, balance recalculation, loading/error states, refetches on revision change.
- [ ] 3.4.2 GREEN: Create `src/modules/invoices/usePayments.ts` — hook.
- [ ] 3.4.3 RED: Write `src/modules/invoices/PaymentForm.test.tsx` — full payment → paid, partial → partially_paid, reject future date, reject overpayment with remaining balance message, reject zero, void with confirm, void restores balance, cancel void preserves.
- [ ] 3.4.4 GREEN: Create `src/modules/invoices/PaymentForm.tsx` — payment form with amount/date/method, void button with `ConfirmDialog`.
- [ ] 3.4.5 GREEN: Integrate `PaymentForm` into `InvoiceDetailPage`. _(Wiring — behavior tested via PaymentForm and InvoiceDetailPage tests.)_
- [ ] 3.4.6 REFACTOR: Verify all payment invariant scenarios pass.
